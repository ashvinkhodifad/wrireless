...
class sale_order(models.Model):
    ...
    def import_sales_order(self,seller,marketplaceids=[],created_before='',created_after=''):
        ...
        mws_obj=Orders(access_key=seller.access_key and str(seller.access_key) or False,
                          secret_key=seller.secret_key and str(seller.secret_key) or False,
                          account_id=seller.merchant_id and str(seller.merchant_id) or False,
        ...
        if not created_after:
            if seller.order_last_sync_on:
                earlier_str= seller.order_last_sync_on - timedelta(int(seller.last_import_fbm_order_days) or 0)
    ...
    def update_order_status(self,seller,marketplaceids=[]):
        proxy_data=seller.get_proxy_server()
        mws_obj=Feeds(access_key=seller.access_key and str(seller.access_key) or False,
                          secret_key=seller.secret_key and str(seller.secret_key) or False,
                          account_id=seller.merchant_id and str(seller.merchant_id) or False,
                          region=seller.country_id.amazon_marketplace_code or seller.country_id.code,
                          auth_token=seller.auth_token and str(seller.auth_token) or False,
    ...
    def import_sales_order_by_flat_report(self,seller,marketplaceids=[],start_date=False,end_date=False,status=('Unshipped','PartiallyShipped')):
        ...
        mws_obj = Reports(access_key=seller.access_key and str(seller.access_key) or False,
                          secret_key=seller.secret_key and str(seller.secret_key) or False,
                          account_id=seller.merchant_id and str(seller.merchant_id) or False,
                          region=seller.country_id.amazon_marketplace_code or seller.country_id.code,
                          auth_token=seller.auth_token and str(seller.auth_token) or False)
