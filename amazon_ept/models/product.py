...
#         return res
    ...
    def search_product(self,seller_sku):
        ...
        if self.env.ref('product.product_comp_rule').active == True:
            ...
            product = product_obj.search(['|',('company_id','=',False),('company_id','=',company_id),('default_code','=',seller_sku)])
            
            if len(product) > 1:
                raise Warning('Duplicate Product Found For Sku (%s)'%(seller_sku))
            
            if not product:
                product = product_obj.search(['|',('company_id','=',False),('company_id','=',company_id),('default_code','=',seller_sku),('active','=',False)])
...
#             data.append(self.get_tsd_language(product))      
        ...
        product.manufacturer and data.append("<Manufacturer>%s</>"%(cgi.escape(product.manufacturer)))#.encode("utf-8") 
