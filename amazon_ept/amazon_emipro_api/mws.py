...
from ..pyAesCrypt.crypto import decryptFile
import os
import glob
import csv
...
class MWS(object):
    ...
    def __init__( self, access_key, secret_key, account_id, region='US', domain='', uri="", version="", auth_token="", proxies={}):
        ...
        europe = ['DE','ES','FR','IN','IT','UK','TR']
        north_america = ['BR','CA','MX','US']
        far_east_region = ['AU','JP']
        file_find_path = ''
        filename = ''
        ...
        if access_key and secret_key:
            ...
            if region in europe:
                file_find_path = cur_dir+'/**/'+'europe'+'.csv.aes'
                
            elif region in north_america:
                file_find_path = cur_dir+ '/**/'+'north_america'+'.csv.aes'
            
            elif region in far_east_region:
                file_find_path = cur_dir+'/**/'+'far_east'+'.csv.aes'
                
            else:
                return

            for filename in glob.iglob(file_find_path, recursive=True):
                filename = filename
                break
            
            if filename:
                
                decryptFile(filename, '/tmp/dataout.csv', '', bufferSize)

                reader = csv.DictReader(open('/tmp/dataout.csv', "rU"), delimiter="\t")

                if reader:
                    if reader.fieldnames and len(reader.fieldnames) == 2:
                        for line in reader:
                            self.access_key = line.get(reader.fieldnames[0])
                            self.secret_key = line.get(reader.fieldnames[1])
