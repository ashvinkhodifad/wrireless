...
class product_listing(models.Model):
    ...
    def update_active_listing(self, instance, item, prod_listing_id):
        ...
        if prod_listing_id:
            ebay_categ1 = False
            ebay_categ2 = False
            ebay_storecateg1=False
            ebay_storecateg2=False
            categ1 = item.get('PrimaryCategory',{}).get('CategoryID')
            categ2 = item.get('SecondaryCategory',{}).get('CategoryID')
            storecateg1=item.get('Storefront',{}).get('StoreCategoryID')
            storecateg2=item.get('Storefront',{}).get("StoreCategory2ID")
            if categ1:
            ...
            if storecateg1:
                ebay_storecateg1 = ebay_category_master_obj.search([('ebay_category_id','=',storecateg1),('is_store_category','=',True)],limit=1)
            if storecateg2:
                ebay_storecateg2 = ebay_category_master_obj.search([('ebay_category_id','=',storecateg2),('is_store_category','=',True)],limit=1)
            prod_listing_id.ebay_product_tmpl_id.write({'category_id1':ebay_categ1 and ebay_categ1.id,
                                                        'category_id2':ebay_categ2 and ebay_categ2.id,
                                                        'store_categ_id1':ebay_storecateg1 and ebay_storecateg1.id,
                                                        'store_categ_id2':ebay_storecateg2 and ebay_storecateg2.id})
        return True
