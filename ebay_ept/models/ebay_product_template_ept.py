...
#             return {'domain':domain}
    ...
    def get_shipping_details(self,ebay_config_template):
        ...
        if ebay_config_template.ship_type=='Calculated':
            calculate_rate.update({
                'PackagingHandlingCosts': ebay_config_template.handling_cost or 0.0,
                "PackageDepth": {'#text':ebay_config_template.ebay_package_depth},
                "PackageLength": {'#text':ebay_config_template.ebay_package_length},
                "PackageWidth": {'#text':ebay_config_template.ebay_package_width},
            })
    ...
    def prepare_product_dict(self,ebay_product_template,ebay_config_template,instance,product_type,publish_in_ebay,schedule_time,listing_type=False):
        ...
        post_code = instance and instance.post_code or ''
        condition = ""
        ...
        condition and sub_dict.update({'ConditionDescription':(cgi.escape(condition_description).encode("utf-8")).decode("iso-8859-1") ,'ConditionID' : condition})
        if product_type != 'individual':
            sub_dict.update({'Description': prod_desc})
        sub_dict.update({'Currency' :ebay_config_template.start_price_id.currency_id.name,})
        ...
        item_location_country_id = instance.item_location_country or False
        item_location_name = instance.item_location_name or ''
        
        item_location_country_id and sub_dict.update({'Country': item_location_country_id.code or ''})
        post_code and sub_dict.update({'PostalCode': post_code})
        item_location_name and sub_dict.update({'Location': item_location_name})
        ...
        if ebay_config_template.return_policy=='ReturnsAccepted':
            sub_dict.get('ReturnPolicy').update({
                'RefundOption':ebay_config_template.refund_option_id.name,
                'ReturnsWithinOption':ebay_config_template.return_days_id.name,
                'Description': (cgi.escape(ebay_config_template.return_policy_description or '').encode("utf-8")).decode("iso-8859-1"),
                'ShippingCostPaidByOption':ebay_config_template.refund_shipping_cost_id.name
        })
    ...
    def prepare_variation_product_dict(self,ebay_product_template,ebay_config_template,instance,product_type,publish_in_ebay,schedule_time):
        ...
        for line in ebay_product_template.product_tmpl_id.attribute_line_ids:
            ...
            ebay_name = attribute.ebay_name or ""
            simple_name= attribute.name or ""
            name_value_list.append({'Name':ebay_name.encode("utf-8").decode("iso-8859-1")  or simple_name.encode("utf-8").decode("iso-8859-1") ,'Value':values})
        ...
        for variant in ebay_product_template.ebay_variant_ids:
            ...
            #for encode SKU
            default_code = variant.ebay_sku
            default_code = (cgi.escape(default_code).encode("utf-8")).decode("iso-8859-1")

            variation.update({'SKU':default_code})
            ...
            for value in variant.product_id.attribute_value_ids:
                ebay_name = value.attribute_id.ebay_name or ""
                simple_name= value.attribute_id.name or ""
                name_value_list.append({'Name':ebay_name.encode("utf-8").decode("iso-8859-1")  or  simple_name.encode("utf-8").decode("iso-8859-1"),'Value':value.name})
        ...
        list_of_variation_pictures.append({'VariationSpecificPictureSet':picture_set,'VariationSpecificName':(cgi.escape(ebay_product_template.attribute_id.name).encode("utf-8")).decode("iso-8859-1")})
                    ...
                    if hasattr(variant.product_id, attribute.field_id.name):
                        if getattr(variant.product_id, attribute.field_id.name):
                            if attribute.field_id.ttype =='many2one':
                                ...
                                prod_desc = prod_desc.replace(attribute.text,str(value.name))
                            else:
                                value = getattr(variant.product_id, attribute.field_id.name)
                                prod_desc = prod_desc.replace(attribute.text,str(value))                                
                        else:
                            prod_desc = prod_desc.replace(attribute.text,'')
                            
        if product_type == 'individual' and instance.use_dynamic_desc and desc_template_id:
            prod_desc = prod_desc.encode("utf-8").decode("iso-8859-1")
        sub_dict.update({'Description' : prod_desc})
...
class ebay_product_product_ept(models.Model):
    ...
    def export_stock_levels_ebay(self, instance, ebay_products=False):
        ...
        for ebay_product in ebay_products:            
            ...
            if history_record and history_record.last_updated_qty==int(stock) and not ebay_product.ebay_stock_type:  
                continue
