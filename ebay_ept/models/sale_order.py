...
class sale_order(models.Model):
                        ...
                        for move in picking.move_lines:
                            if move.state in  ['confirmed','partially_available']:
                                ...
                                if remaining_qty > 0.0:
                                    stock_move_line_obj.create({'product_id':move.product_id.id,
    ...
    def create_or_update_partner(self,order,instance):
        ...
        #for set state value
        country = country_obj.search([('code','=',address_info.get('Country'))])               
        state=address_info.get('StateOrProvince',False) 
        state_id = False
        if state:
            if country:
                result_state = state_obj.search(['|',('code','=',state),('name','=',state),('country_id','=',country.id)],limit=1)
            else:
                result_state = state_obj.search(['|',('code','=',state),('name','=',state)],limit=1)
            if result_state:
        ...
        if exist_partner:
            ...
            if not exist_partner:
                # Need to remove "property_product_pricelist" during create new delivery type partner - Remainin
                ...
                partner and return_partner.update({'invoice_address':partner.id,'delivery_address':partner.id,'pricelist_id': instance.pricelist_id and instance.pricelist_id.id}) #partner.property_product_pricelist and partner.property_product_pricelist.id
            else:
                return_partner.update({'invoice_address':exist_partner.id,'delivery_address':exist_partner.id,'pricelist_id': instance.pricelist_id and instance.pricelist_id.id}) #exist_partner.property_product_pricelist and exist_partner.property_product_pricelist.id
        else:
