...
class ebay_instace_ept(models.Model):
    ...
    item_location_country = fields.Many2one("res.country","Item Location",help="Select the country for the Item Location.")
    item_location_name = fields.Char(string="Item Location(City, State) Name",help="Item Location(City, State) name.")
    ...
    def create(self,vals):        
        ...
        try:
            if instance:
                ...
                para = {'OutOfStockControlPreference':'true'} if instance.allow_out_of_stock_product else {'OutOfStockControlPreference':'false'}
                api.execute('SetUserPreferences',para)
                api.response.dict()
        except Exception:
            instance.unlink()
    ...
    def send_ebay_invoice_via_email(self,args={}):
        ...
        if instance_id:
            instances = instance_obj.search([('id', '=', instance_id),('state','=','confirmed')],limit=1)
            if not instances: 
                return False
            
            email_template = (instances.send_invoice_template_id) if instances.send_invoice_template_id else self.env.ref('account.email_template_edi_invoice', False)
            for instance in instances:
